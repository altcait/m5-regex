...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean hasCapitalLetter = Pattern.matches(".*[A-Z]+.*", str);
        final boolean hasNumber = Pattern.matches(".*\\d+.*", str);
        final boolean meetsMinLength = str.length() >= minLength;

        return hasLowerCase && hasCapitalLetter && hasNumber && meetsMinLength;
    }
